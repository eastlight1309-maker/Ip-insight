"""IP Insight — Streamlit UI (패키지 내부 모듈).
...
로컬 개발에서는 `streamlit run frontend/app.py` 가 이 main() 을 호출한다.
"""
...
from . import data_loader, dataiku_io, llm, service
from .config import ALLOWED_LLM_CANDIDATES, settings
...
def main() -> None:
    ...
    st.caption("Derwent DWPI 특허 엑셀 업로드 → LLM 기반 인사이트 → PPT 리포트")
    ...
    with st.expander(
        f"미리보기 & 컬럼 매핑 (총 {len(raw)}행, 자동 인식 {len(data_loader.detect_mapping(raw))}개 컬럼)",
        expanded=True,
    ):
